...
<div class="modal fade bd-example-modal-lg" id="memrship_modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<h2 class="modal-title" id="exampleModalLabel">Membership Details</h2>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<div id="membership_details"></div>
			</div>
...
<script>
	var table=$('#user_list').DataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": "<?php echo site_url("teacher/get_teacher_list"); ?>",
		"ordering":false
    } );
	function delete_user(delete_id){
		...
		function(){
			$.ajax({
				type: "POST",
				url: "<?php echo site_url("teacher/delete_teacher"); ?>", 
	...
	function view_membership(id){
		$(".preloader").show();
		$.ajax({
			type: "POST",
			url: "<?php echo site_url("teacher/get_membership_details"); ?>", 
			data: {id:id}, 
			dataType: "html",
			success: function(data){
				$(".preloader").hide();
				$("#membership_details").html(data);
				$("#memrship_modal").modal("show");
